...
const validateAppointments = yup.object().shape({
  scheduleId: yup.number().integer().required(),
...
const validateprocedureId = yup.object().shape({
  id: yup.string().uuid("O id de audiência informado é inválido.").required(),
});

const validateprocedureUpdate = yup.object().shape({
